...
interface Record { id: string; name: string; gender: string; question: string; hexagram: any; result: any; createdAt: string; user?: { username: string }; userId: string }
...
async function fetchList() {
  ...
  try {
    ...
    const res: any = await request.get('/admin/divination-records', { params })
...
async function openDetail(row: Record) {
  try {
    const res: any = await request.get(`/admin/divination-records/${row.id}`)
    detail.value = res
...
async function handleDelete(row: Record) {
  try {
    ...
    await request.delete(`/admin/divination-records/${row.id}`)
...
async function handleBatchDelete() {
  ...
  try {
    ...
    deleting.value = true
    await Promise.all(selectedIds.value.map(id => request.delete(`/admin/divination-records/${id}`).catch(() => {})))
  ...
  <div>
    ...
    <el-card shadow="hover">
      ...
      <el-table :data="list" v-loading="loading" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column label="所问之事" min-width="200"><template #default="{ row }">{{ row.question?.slice(0, 60) || '-' }}</template></el-table-column>
        ...
        <el-table-column label="操作" min-width="140">
          <template #default="{ row }">
            <el-button size="small" @click="downloadPdf(row.id, '/divination', `八卦问事_${row.name}`)">PDF</el-button>
    ...
    <el-dialog v-model="showDetail" title="八卦问事详情" width="640px">
      <div v-if="detail" style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        ...
        <div style="grid-column:span 2"><label>所问之事</label><div>{{ detail.question }}</div></div>
        <div style="grid-column:span 2" v-if="detail.hexagram"><label>卦象</label><div>{{ detail.hexagram.originalSymbol }} {{ detail.hexagram.originalName }}<span v-if="detail.hexagram.changedName"> → {{ detail.hexagram.changedSymbol }} {{ detail.hexagram.changedName }}</span></div></div>
